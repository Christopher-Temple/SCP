...
require("/scripts/scp_util.lua")

QD = nil
GD = nil

function init()
	GD = root.assetJson("/interface/windowconfig/questList.config")
	QD = root.assetJson("/interface/windowconfig/data.config")
	
	for _, file in ipairs(QD.questlineFiles) do
		local temp = root.assetJson(file)
		QD = scputil.MergeTable(QD, temp)
	end
...
function populateQueslineList()
	...
	local questlineStatuses = status.statusProperty("scpQuestlinesComplete", "")
	...
	for _, tbl in ipairs(QD.lines) do
		...
		if available then
			...
			widget.setImage(path..".banner", "/interface/questList/banners/"..tbl.id..".png:default")
		...
		elseif not tbl.secret then
			...
			widget.setImage(path..".banner", "/interface/questList/banners/"..tbl.id..".png:unavailable")
		end
	end
	
	status.setStatusProperty("scpQuestlinesComplete", questlineStatuses)
end
...
function questlineSelected()
	...
	if listItem then
		...
		local buttons = {
			base = "/interface/questList/questlineListOverlay.png:selected",
			hover = "/interface/questList/questlineListOverlay.png:selectedHover",
		}
		widget.setButtonImages(listItem..".overlay", buttons)
		
		if self.lastSelected and self.lastSelected ~= listItem then
			local lastButtons = {
				base = "/interface/questList/questlineListOverlay.png:default",
				hover = "/interface/questList/questlineListOverlay.png:hover",
			}
	...
	for questID, path in pairs(self.questListIDs) do
		...
		if config then
			if player.hasCompletedQuest(questID) then
				...
				widget.setImage(path..".status", "/interface/questList/status.png:complete")
				
			elseif player.hasQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "cyan")
				widget.setImage(path..".status", "/interface/questList/status.png:inProgress")
				
			elseif player.canStartQuest(questID) then
				widget.setText(path..".questName", tostring(config.title))
				widget.setFontColor(path..".questName", "yellow")
				widget.setImage(path..".status", "/interface/questList/status.png:available")
		
			else
				if config.secret then
					widget.setText(path..".questName", "???")
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/questList/status.png:secret")
				else
					widget.setText(path..".questName", tostring(config.title))
					widget.setFontColor(path..".questName", "red")
					widget.setImage(path..".status", "/interface/questList/status.png:unavailable")
				end
			end
		else
				...
				for sublineNum, subline in ipairs(lineTbl.sublines) do
					for questNum, questID in ipairs(QD.sublines[subline]) do
						...
						if questNum == #QD.sublines[subline] then
							...
							widget.setImage(portrait, "/interface/questList/givers/"..subline..".png")
...
function questSelected()
	...
	if listItem then
		...
		if id == GD.portraitSpacingString then
			...
			widget.setImage(listItem..".bg", "/interface/questList/questBackground.png:selected")
			...
			if self.lastSelected and self.lastSelected ~= listItem then
				widget.setImage(self.lastSelected..".bg", "/interface/questList/questBackground.png:default")
			end
...
function questButton()
	if widget.active("questlineList") then
		...
		if path then
			...
			widget.setImage("questlineBanner", "/interface/questList/banners/"..questline..".png:default")
